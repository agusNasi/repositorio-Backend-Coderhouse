...
class ProductFileDao {
  ...
  async getById(id) {
    ...
    try {
      ...
      const selectedProduct = savedProducts.find(
        (prod) => prod.id === idNumber
      );
      if (!selectedProduct) {
  ...
  async add(product) {
    try {
      const savedProducts = await this.getProducts();
      const DuplicatedProduct = savedProducts.find(
        (item) => item.code == product.code
      );
      if (DuplicatedProduct) {
        throw new Error(
          `ERROR: Unable to add. The next code has been already added: ${product.code}`
        );
      }
      if (
        !product.title ||
      ...
      const newId =
        savedProducts.length > 0
          ? savedProducts[savedProducts.length - 1].id + 1
          : 1;
      const newProduct = {
        id: newId,
